...
apt-get -y purge `dpkg -l | awk '/mysql/ {print $2}'` >/dev/null
apt-get -y purge `dpkg -l | awk '/keystone/ {print $2}'` >/dev/null
apt-get -y purge `dpkg -l | awk '/glance/ {print $2}'` >/dev/null
...
do_dsh apt-get -y purge `dpkg -l | awk '/nova/ {print $2}'` >/dev/null
do_dsh rm -rfv /etc/nova /var/lib/nova/ /var/log/nova/ >/dev/null
do_dsh apt-get -y autoremove >/dev/null
 
do_dsh dpkg -P -a >/dev/null
